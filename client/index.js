...
function drawChart(chart_data,canvas_id,chart_id=""){
    ...
    if(chart_objects.hasOwnProperty(chart_id)){
        ...
        updata_data = {
            ...
            multiple_yAxes: (myChart.options.hasOwnProperty("scales"))? myChart.options.scales.yAxes[1].display:false,
    ...
    if(Object.keys(updata_data)!=0){
        if(options.hasOwnProperty("scales")){
            // update multiple axes
            options.scales.yAxes[1].display = updata_data.multiple_yAxes

            // update y axis
            options.scales.yAxes[0].scaleLabel.labelString = updata_data.y
        }

        // update data value(aggre)
        updata_data.data.forEach(function(data,index){
            datasets[index].data = data
        })

        // update labels (sort)
        labels = updata_data.labels
    }
