    ...
    for point in data.values():
        for key,value in point.items():
            if key in dataInfo['nominal'] or key in dataInfo['temporal']: 
                if key!='' and key!='number': colFeatures[key].add(value) 
...
def enumerateVis(dataInfo,dataName = 'AQ'): # based on DeepEye P.8  # y不能是nomial
    ...
    def getSubgroup(self,y_aggre=""):
        ...
        for key in list(colFeatures[self.x]):
            if key in self.points:
                y_aggre = self.y_aggre if y_aggre =="" else y_aggre
...
def init(data=None):
    ...
    if presave:
        ...
        try:
            ...
            dataInfos = read_dill(dataInfos_path)
